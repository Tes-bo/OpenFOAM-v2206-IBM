...
//------------------------------------------------------------   
    ...
    forAll(bMesh, patchI)
    {
        if(isA<wallPolyPatch>(bMesh[patchI])||bMesh[patchI].isWall())
        {
...
//------------------------------------------------------------   
    ...
    forAll(mesh_.boundary(), patchi)
    {
        ...
        if (isA<wallFvPatch>(patch))//  sajad
        {
        ...
        else if(mesh_.boundary()[patchi].isWall())//sajad
        {
            ypatch = 1/mesh_.boundary()[patchi].deltaCoeffs();
        }
        else
        {
            ypatch = 0.0;
