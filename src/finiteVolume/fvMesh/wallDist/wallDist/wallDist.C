...
:
    ...
    (
        ...
        mesh,
        dimensionedScalar("yIb" & patchTypeName_, dimLength, SMALL),
        patchDistMethod::patchTypes<scalar>(mesh, patchIDs_)
    ),
...
:
    ...
    (
        mesh,
        mesh.boundaryMesh().findPatchIDsWallIb<wallPolyPatch>(),//sajad
        patchTypeName
    )
{}
...
Foam::volScalarField& Foam::wallDist::yIb()//sajad
{
     pdm_->correct(y_);
     return y_;
