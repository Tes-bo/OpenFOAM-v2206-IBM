...
:
    ibEpsilonWallFunctionFvPatchScalarField(p, iF),
...
:
    ibEpsilonWallFunctionFvPatchScalarField(p, iF),   // Do not read mixed data
...
:
    ibEpsilonWallFunctionFvPatchScalarField(p, iF), // Do not map mixed data.  Set patchType later
    immersedBoundaryFieldBase<scalar>
    (
        p,
        ptf.setDeadValue(),
...
:
    ibEpsilonWallFunctionFvPatchScalarField(ptf),
    immersedBoundaryFieldBase<scalar>
    (
        ptf.ibPatch(),
        ptf.setDeadValue(),
...
:
    ibEpsilonWallFunctionFvPatchScalarField(ptf, iF),
    immersedBoundaryFieldBase<scalar>
    (
        ptf.ibPatch(),
        ptf.setDeadValue(),
...
void immersedBoundaryEpsilonWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    // HJ, 20/May/2018
        ibEpsilonWallFunctionFvPatchScalarField::updateCoeffs();
    if (db().foundObject<volScalarField>(turbModel.GName()))
    {
        ibEpsilonWallFunctionFvPatchScalarField::updateCoeffs();
    }
}
...
)
{
    ...
    scalarField& intField = const_cast<scalarField&>(this->primitiveField());

    // Set dead value
    this->setDeadValues(intField);

    ibEpsilonWallFunctionFvPatchScalarField::evaluate(commsType);
}
...
)
{
    ...
    ibEpsilonWallFunctionFvPatchScalarField::manipulateMatrix(matrix);
}
