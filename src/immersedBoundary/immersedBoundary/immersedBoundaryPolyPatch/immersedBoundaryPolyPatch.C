...
//~ (
    ...
    //~ 20
//~ );
//static scalar immersedBoundaryPolyPatch::spanFactor_ = 20;
...
void Foam::immersedBoundaryPolyPatch::calcTriSurfSearch() const
{
    ...
    triSurfSearchPtr_ = new triSurfaceSearch(ibMesh_,true);
}
        ...
        if (bMesh[patchI].coupled())
        {
            if (isA<processorPolyPatch>(bMesh[patchI]))
            {
                if (Pstream::parRun())
                {
                    ...
                    (
                        UPstream::commsTypes::blocking,//Pstream::blocking,
...
void Foam::immersedBoundaryPolyPatch::calcCorrectedGeometry() const
{
    ...
    correctedIbPatchFaceAreasPtr_ = new vectorField(ibPatch().faceAreas());
