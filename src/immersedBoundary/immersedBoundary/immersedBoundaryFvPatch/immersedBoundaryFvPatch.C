...
//~ const Foam::debugs::tolerancesSwitch
//~ Foam::immersedBoundaryFvPatch::nonOrthogonalFactor_
//~ (
    //~ "immersedBoundaryNonOrthogonalFactor",
    //~ 0.1
//~ );
//static scalar immersedBoundaryFvPatch::nonOrthogonalFactor_ = 0.01;
...
void Foam::immersedBoundaryFvPatch::makeCf(slicedSurfaceVectorField& Cf) const
{
    ...
    forAll(mesh.boundary(), patchI)
    {
        if(isA<immersedBoundaryFvPatch>(mesh.boundary()[patchI]))
        {
            ...
            Cf.boundaryFieldRef()[patchI].UList::operator=//Cf.boundaryField()[patchI].UList::operator=//sajad
                (
...
void Foam::immersedBoundaryFvPatch::makeSf(slicedSurfaceVectorField& Sf) const
{
    ...
    forAll(mesh.boundary(), patchI)
    {
        if(isA<immersedBoundaryFvPatch>(mesh.boundary()[patchI]))
        {
            ...
            Sf.boundaryFieldRef()[patchI].UList::operator=//Sf.boundaryField()[patchI].UList::operator=//sajad
                (
                    vectorField::subField
                    (
...
void Foam::immersedBoundaryFvPatch::makeC(slicedVolVectorField& C) const
{
    ...
    forAll(mesh.boundary(), patchI)
    {
        if(isA<immersedBoundaryFvPatch>(mesh.boundary()[patchI]))
        {
            ...
            C.boundaryFieldRef()[patchI].UList::operator=//C.boundaryField()[patchI].UList::operator=//sajad
                (
                    vectorField::subField
                    (
                        curIbPatch.ibPolyPatch().ibPatch().faceCentres(),
...
) const
{
    ...
    // Scale internalField
    scalarField& phiIn = phi.primitiveFieldRef();
    ...
    forAll (mesh.boundary(), patchI)
    {
        if (!isA<immersedBoundaryFvPatch>(mesh.boundary()[patchI]))
        {
                forAll(phibf[patchI],counter)
                phibf[patchI][counter] *=
                mesh.boundary()[patchI].patchSlice(ibAreaRatio)[counter];
        }
    }
