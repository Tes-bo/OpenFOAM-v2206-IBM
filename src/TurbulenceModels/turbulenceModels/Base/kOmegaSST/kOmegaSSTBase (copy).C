...
) const
{
    ...
    (
        ...
        (
            max
            (
                (scalar(1)/betaStar_)*sqrt(k_)/(omega_*ys_),
                scalar(500)*(this->mu()/this->rho_)/(sqr(ys_)*omega_)
            ),
            (4*alphaOmega2_)*k_/(CDkOmegaPlus*sqr(ys_))
        ),
        scalar(10)
    );
...
tmp<volScalarField> kOmegaSSTBase<BasicEddyViscosityModel>::F2() const
{
    ...
    (
        max
        (
            (scalar(2)/betaStar_)*sqrt(k_)/(omega_*ys_),
            scalar(500)*(this->mu()/this->rho_)/(sqr(ys_)*omega_)
        ),
        scalar(100)
    );
...
tmp<volScalarField> kOmegaSSTBase<BasicEddyViscosityModel>::F3() const
{
    ...
    (
        150*(this->mu()/this->rho_)/(omega_*sqr(ys_)),
        scalar(10)
    );
